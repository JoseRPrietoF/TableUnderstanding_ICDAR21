...
class normalizeTensor(object):
    ...
    def __call__(self, sample):
        if torch.is_tensor(sample["img"]):
            ...
            if (
                not len(self.mean) == sample["img"].shape[0]
                or not len(self.std) == sample["img"].shape[0]
            ):
                raise ValueError(
                    "mean and std size must be equal to the number of channels of the input tensor."
                )
            for i, t in enumerate(sample["img"]):
                t.sub_(self.mean[i]).div_(self.std[i] + np.finfo(float).eps)
        else:
            raise TypeError(
...
class normalizeArray(object):
    ...
    def __call__(self, sample):
        if type(sample["img"]) is np.ndarray:
            if self.mean is None or self.std is None:
                self.mean = []
                self.std = []
                for t in sample["img"]:
...
class elastic(object):
    ...
    def __call__(self, sample):
        ...
        if torch.rand(1)[0] < self.prob:
            ...
            if np.ndim(sample["img"]) > 2:
                for t in sample["img"]:
                    t[:] = map_coordinates(t, indices, order=1).reshape(shape)
            else:
                sample["img"] = map_coordinates(
                    sample["img"], indices, order=1
                ).reshape(shape)
            """Labels"""
            if np.ndim(sample["img_cells"]) > 2:
                for t in sample["img_cells"]:
                    t[:] = map_coordinates(t, indices, order=1).reshape(shape)
            else:
                sample["img_cells"] = map_coordinates(
                    sample["img_cells"], indices, order=1
                ).reshape(shape)
    ...
    def __call__(self, sample):
        ...
        if (T == np.eye(3)).all():
            ...
            if np.ndim(sample["img"]) > 2:
                ...
                sample["img"] = affine_transform(sample["img"], T)
            """Labels"""
            if np.ndim(sample["img_cells"]) > 2:
                ...
                sample["img_cells"] = affine_transform(
                    sample["img_cells"], T, mode="nearest", order=0
                )
            return sample
